...
namespace GUI
{
	public partial class Form1 : Form
	{
		...
		private void button2_Click(object sender, EventArgs e)
		{
			...
			while (Path.GetFileName(picture) != "CEK FORM 2")
            {
			...
			Form resForm = BFS.getForm();

			using (Graphics gfx = resForm.CreateGraphics())
			{
				using (Bitmap bmp = new Bitmap(resForm.Width, resForm.Height, gfx))
				{
					resForm.DrawToBitmap(bmp, new Rectangle(0, 0, resForm.Width, resForm.Height));
					bmp.Save(picture+"result.jpg");
				}
			}
			pictureBox1.ImageLocation = picture + "result.jpg";
			//BFS.printArr(ans);
			/*
			while (true)
            {
